...
# Workflow filename used to find the last successful run (from_last_run mode).
# Override via WORKFLOW_FILENAME env var when the calling workflow has a different name.
WORKFLOW_FILENAME = os.environ.get("WORKFLOW_FILENAME", "doc-sync.yml")

# Resolve calling repo root:
#   - In GitHub Actions: GITHUB_WORKSPACE is set to the calling repo's checkout root.
#   - Locally (for dev/testing): fall back to two directories above this file
#     (.../doc-sync/scripts/main.py -> .../doc-sync/scripts -> .../doc-sync -> fallback root).
_GITHUB_WORKSPACE = os.environ.get("GITHUB_WORKSPACE", "")
_REPO_ROOT = Path(_GITHUB_WORKSPACE) if _GITHUB_WORKSPACE else Path(__file__).resolve().parents[2]

# Path to the doc-map JSON in the calling repo.
# Override DOC_MAP_RELATIVE_PATH to use a non-default location.
DOC_MAP_PATH = str(_REPO_ROOT / os.environ.get("DOC_MAP_RELATIVE_PATH", "doc-map.json"))
